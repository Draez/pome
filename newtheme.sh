...
echo "${boldwhite}Please note: This script is designed to work with dudestack and pome. See https://github.com/digitoimistodude${txtreset} "
...
for i in `grep -rl pome * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;pome;${THEMENAME};" $i $i; done
for i in `grep -rl pome * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;pome;${THEMENAME};" $i $i; done
for i in `grep -rl air * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;pome;${THEMENAME};" $i $i; done
...
On top of features included in [digitoimistodude/pome](https://github.com/digitoimistodude/pome), this project contains:
